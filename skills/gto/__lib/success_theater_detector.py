...
class SuccessTheaterFlag:
    ...
    evidence: dict[str, Any]  # What made us flag this
...
class SuccessTheaterResult:
    ...
    is_healthy: bool  # True if no theater detected
    flags: list[SuccessTheaterFlag]
    overall_verdict: str  # "GENUINE" / "SUSPECT" / "THEATER"
        ...
        return [
            {
                ...
                "effort_estimate_minutes": 5,
                "advisory": True,  # Advisory enforcement heuristic
            }
            for flag in self.flags
        ]
...
class SuccessTheaterDetector:
    ...
    def __init__(self, project_root: Path | None = None):
        ...
        """
        self.project_root = Path(project_root or Path.cwd()).resolve()
        self.state_dir = self.project_root / ".evidence" / f"gto-state-{_get_terminal_id()}"
        self.assertions_path = self.project_root / ".claude" / "skills" / "gto" / "evals" / "gto_assertions.py"
    ...
    def check(
        ...
        Args:
            artifact: GTO artifact dict (from save_json_artifact)
            metadata: Orchestrator metadata dict

        Returns:
            SuccessTheaterResult with any detected flags
        """
        flags: list[SuccessTheaterFlag] = []

        # Pattern 1: Health score 100 but assertions were skipped
        if artifact:
            health_score = artifact.get("health_score") or artifact.get("overall") or artifact.get("health", {}).get("overall_score")
            if health_score == 100:
                flag = self._check_assertions_ran(artifact)
                if flag:
                    flags.append(flag)

        # Pattern 2: No gaps but detectors were disabled
        if artifact:
            gap_count = artifact.get("total_gap_count", 0)
            if gap_count == 0:
                flag = self._check_quiet_means_disabled(metadata)
                if flag:
                    flags.append(flag)

        # Pattern 3: health_score present but health report is absent or thin
        if artifact:
            flag = self._check_empty_health(artifact)
            if flag:
                flags.append(flag)

        # Pattern 4: Artifact timestamp but no recent assertions run
        if artifact:
            flag = self._check_stale_assertions(artifact)
            if flag:
                flags.append(flag)
    ...
    def _check_assertions_ran(self, artifact: dict[str, Any]) -> SuccessTheaterFlag | None:
        """Check if assertions actually ran for a 100% health score."""
        # Check if assertions_passed field is present and valid
        assertions_passed = artifact.get("assertions_passed")
        assertions_total = artifact.get("assertions_total")

        if assertions_passed is not None and assertions_total is not None:
            # Assertions were explicitly recorded
            if assertions_passed < assertions_total:
                return SuccessTheaterFlag(
                    pattern="assertions_failed",
                    severity="HIGH",
        ...
        # Check metadata for viability failure that was overridden
        metadata = artifact.get("metadata", {})
        viability = metadata.get("viability_check", {})
        if viability.get("is_viable") is False:
            return SuccessTheaterFlag(
        ...
        # health_score = 100 with no assertions data at all
        if assertions_passed is None and assertions_total is None:
            # Only flag if there's evidence assertions should have run
            if artifact.get("total_gap_count", 0) == 0:
                return SuccessTheaterFlag(
                    pattern="no_assertions",
                    severity="MEDIUM",
                    description="health_score=100 but no assertion data found — assertions may not have run",
                    evidence={
                        "has_assertions_data": False,
                        "total_gaps": artifact.get("total_gap_count"),
                    },
                    recommendation="Run gto_assertions.py to verify health",
                )
    ...
    def _check_quiet_means_disabled(self, metadata: dict[str, Any] | None) -> SuccessTheaterFlag | None:
        ...
        # Check if subagents were disabled
        subagents_disabled = metadata.get("subagents_disabled", False)
        ...
        # Check if detectors were skipped
        detector_count = metadata.get("detector_count", 0)
        if detector_count < 5:
            return SuccessTheaterFlag(
                pattern="few_detectors",
    ...
    def _check_empty_health(self, artifact: dict[str, Any]) -> SuccessTheaterFlag | None:
        """Check if health_score is present but health report is absent or thin."""
        has_health_score = artifact.get("health_score") or artifact.get("overall")
        has_health_report = artifact.get("health_report") or artifact.get("health")
    ...
    def _check_stale_assertions(self, artifact: dict[str, Any]) -> SuccessTheaterFlag | None:
        """Check if artifact exists but assertions haven't run recently."""
        artifact_time = artifact.get("timestamp", "")
        if not artifact_time:
            return None
        ...
        # If artifact is older than 24 hours, flag as potentially stale
        if datetime.now() - artifact_dt > timedelta(hours=24):
            return SuccessTheaterFlag(
                pattern="stale_artifact",
                severity="LOW",
